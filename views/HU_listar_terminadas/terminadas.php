...
<!-- Buscador SQL --> 
    <nav class="navbar navbar-light justify-content-between" style="margin-bottom: 5vh;">
      ...
      <?php if(isset($_GET['enviar'])){
        ...
        $buscadorSQL = $con->query("SELECT *, max(opcion.cantidad_votos) as 'ganador' from votacion, opcion WHERE votacion.id = opcion.votacion_id and votacion.titulo LIKE '%$busqueda%' and votacion.esta_activa = 0 group by votacion.id ORDER BY `ganador` DESC;");
      } ?>
